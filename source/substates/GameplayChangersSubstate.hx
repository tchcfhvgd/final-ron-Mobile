...
class GameplayChangersSubstate extends MusicBeatSubstate
{
	...
	private var grpTexts:FlxTypedGroup<gameassets.AttachedText>;

	function getOptions()
	{
		ClientPrefs.gameplaySettings.set("Scroll Type", "constant");
	...
	public function new()
	{
		...
		grpTexts = new FlxTypedGroup<gameassets.AttachedText>();
		add(grpTexts);
		...
		for (i in 0...optionsArray.length)
		{
			...
			if(optionsArray[i].type == 'bool') {
				...
				var valueText:gameassets.AttachedText = new gameassets.AttachedText('' + optionsArray[i].getValue(), optionText.width + 80, true, 0.8);
